...
const Meals = () => {
  ...
  const handleIncrement = () => {
    setOrderquantity(prev => Math.min(prev+1,popupContent.available_count)); // prevents going above available count
  };
...
const fetchRestaurants = async () => {
  try {
    const response = await axios.get("https://4399-91-186-255-241.ngrok-free.app/api/get/all-restaurants-names", {
      headers: {
...
const fetchAllMeals = async () => {
  try {
    const response = await axios.get("https://4399-91-186-255-241.ngrok-free.app/api/all-meals", {
      headers: {
        'Accept': 'application/json',
        'ngrok-skip-browser-warning': 'true',
      }
...
useEffect(() => {
  ...
  if (isLoading) {
    return <div className="loading"> <RotatingLines
    strokeColor="grey"
    strokeWidth="5"
  ...
  return (
    ...
    <div className={`container ${openpopup ? "blurred" : ""}`}  >
      <div className="tabs">
        ...
        <button className="tab" onClick={(e)=>window.location.href="/restaurantslist"}>Restaurant</button>
      </div>
  ...
  <>
    ...
    <div className="grid">
      {allMeals.length > 0 ? (
        allMeals.map((meal) => (
          <div key={meal.id} className="card"  onClick={() => {
            setOpenPopup(true);}}>
...
<div className="grid" >
  {recommendedMeals.length > 0 ? (
    recommendedMeals.map((meal) => (
      <div className="card" key={meal.id} onClick={() => {
        setOpenPopup(true);
    ...
    <div id="popup-content" ref={popupRef}>
      {popupLoading || !popupContent ? (
        ...
        <>
          ...
          <p  style={{  backgroundColor: popupContent.contains_chicken === 0 && popupContent.contains_meat === 0
  ? "lightgreen":"orange",padding:"8px",borderRadius:"6px",color:"#333", fontWeight: "bold", width: "fit-content"}}>
  {popupContent.contains_chicken === 0 && popupContent.contains_meat === 0  ? "Vegetarian"  : "Contains Meat or Chicken"}
</p>
